...
def print_resultados(resultados):
    ...
    for nombre, data in resultados.items():
        if data is None:
            ...
            print(f"{nombre} → pos=({x}, {y}), size=({w}, {h}), conf={conf:.4f}, scale={scale}")
...
, threshold=0.80, debug=False):
    ...
    if img is None:
        print("❌ No se pudo cargar la captura")
        return {}
...
def test_detectar_puntos():
    ...
    res = detectar_puntos(templates, debug=True)

    print_resultados(res)


if __name__ == "__main__":
    test_detectar_puntos()